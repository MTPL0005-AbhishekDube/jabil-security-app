...
// In-memory lock to prevent concurrent rotations for the same facility
...
const rotateFacilityQRs = async (facility) => {
  ...
  // If a rotation is already in progress for this facility, wait for it
  if (rotationLocks.has(facilityId)) {
    logger.debug(`Rotation already in progress for facility ${facility.name}, waiting...`, { facilityId });
    return rotationLocks.get(facilityId);
  }
  ...
  const rotationPromise = (async () => {
    try {
      ...
      // Release the lock
      rotationLocks.delete(facilityId);
    }
  })();

  rotationLocks.set(facilityId, rotationPromise);
